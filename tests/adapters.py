...
def run_multihead_self_attention(
    ...
    mha.q_proj.load_state_dict({"weights": q_proj_weight})
    mha.k_proj.load_state_dict({"weights": k_proj_weight})
    mha.v_proj.load_state_dict({"weights": v_proj_weight})
    mha.o_proj.load_state_dict({"weights": o_proj_weight})
...
def run_rope(
    ...
    rope = RotaryPositionEmbedding(theta=theta, d_k=d_k, max_seq_len=max_seq_len, device=in_query_or_key.device)
...
def run_transformer_block(
    ...
    # Map weight keys to match module's state_dict naming
    key_map = {
        "attn.q_proj.weight": "attn.q_proj.weights",
        "attn.k_proj.weight": "attn.k_proj.weights",
        "attn.v_proj.weight": "attn.v_proj.weights",
        "attn.output_proj.weight": "attn.o_proj.weights",
    ...
    state_dict = {key_map[k]: v for k, v in weights.items()}
...
def run_transformer_lm(
    ...
    # Map state dict keys to module naming
    # Module keys: token_embedding.weights, layers.{i}.*, ln_final.gamma, lm_head.weights
    # State dict keys: token_embeddings.weight, layers.{i}.*, ln_final.weight, lm_head.weight
    # Layer sub-mappings:
    #   attn.q_proj.weight  -> attn.q_proj.weights
    #   attn.k_proj.weight  -> attn.k_proj.weights
    #   attn.v_proj.weight  -> attn.v_proj.weights
    #   attn.output_proj.weight -> attn.o_proj.weights
    #   ln1.weight -> ln1.gamma
    #   ffn.w1.weight -> ffn.w1.weights (etc.)
    #   ln2.weight -> ln2.gamma
    layer_map = {
        "attn.q_proj.weight": "attn.q_proj.weights",
        "attn.k_proj.weight": "attn.k_proj.weights",
    ...
    for k, v in weights.items():
        if k.startswith("layers."):
            parts = k.split(".", 2)  # "layers.0.attn.q_proj.weight" -> ["layers", "0", "attn.q_proj.weight"]
            suffix = parts[2]
            mapped_suffix = layer_map.get(suffix)
            if mapped_suffix is not None:
                state_dict[f"{parts[0]}.{parts[1]}.{mapped_suffix}"] = v
...
def run_rmsnorm(
    ...
    rmsnorm.load_state_dict({"gamma": weights})
...
def run_get_batch(
    ...
    # print(
    #     f"dataset: {dataset}, batch_size: {batch_size}, context_length: {context_length}, device: {device}"
    # )
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    return bpe.train(input_path, special_tokens, vocab_size)
